...
int pedirEnteroRango(int min, int max)
{
	...
	{
		printf(
				"EEROR, el numero debe estar dentro del rango ingresado, Reingrese numero entero: ");
		scanf("%d", &entero);
	}
	return entero;
}
...
int pedirString(char cadena[], char mensaje[], int limite)
{
	...
	if (cadena != NULL && mensaje != NULL)
	{
		...
		{
			printf("ERROR, Ingresar solo letras (maximo %d): ", limite);
			fflush(stdin);
			gets(auxCad);
		}

		convertirPrimerCaracterMayus(auxCad);
...
int validarInt(int *pResultado, char *mensaje, char *mensajeError,
